...
export const ChecklistItem: React.FC<ChecklistItemProps> = ({
  ...
  return (
    <div className={`rounded-lg transition-all duration-200 group/item cursor-pointer ${isChecked
      ...
      <div className="flex items-start gap-4 transition-all duration-200 group">
        ...
        {isEditing ? (
          ...
          <span className={`flex-1 text-base leading-relaxed transition-all ${isChecked
      ...
      {/* Item Examples */}
      {showExamples && (
        <div className="px-6 pb-6 border-t border-primary/20 dark:border-primary/30 pt-6 bg-bg-surface-2 dark:bg-bg-surface-2" style={{ padding: 'var(--space-lg)' }}>
          {isAdminMode && onExamplesChange && onExamplesSave ? (
            ...
            <div className="space-y-3">
                    ...
                    <div className="bg-primary/10 dark:bg-primary/20 border border-primary/30 dark:border-primary/40 rounded-lg p-3">
                      ...
                      <ul className="space-y-1">
                        {item.examples.good.map((ex, i) => (
                          <li key={i} className="text-sm text-text-secondary dark:text-text-secondary font-mono">{ex}</li>
                        ))}
                      </ul>
                    </div>
                  )}
                    ...
                    <div className="bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 rounded-lg p-3">
                      ...
                      <ul className="space-y-1">
                        {item.examples.bad.map((ex, i) => (
                          <li key={i} className="text-sm text-red-700 dark:text-red-400 font-mono">{ex}</li>
                        ))}
                      </ul>
                    </div>
                  )}
                  ...
                  {item.codeExamples.good.length > 0 && (
                    <div className="space-y-2">
                      ...
                      {item.codeExamples.good.map((codeEx, i) => (
                        <div key={i} className="bg-black-russian dark:bg-blue-charcoal rounded-lg p-3 overflow-x-auto border border-border-dark">
                          <div className="text-xs text-loblolly dark:text-regent-gray mb-1">{codeEx.language.toUpperCase()}</div>
              ...
              {!hasExamples && !isAdminMode && (
                <p className="text-xs text-text-muted dark:text-text-muted italic">No examples for this item.</p>
              )}
            </div>
          )}
        </div>
