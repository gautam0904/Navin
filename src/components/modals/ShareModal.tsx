...
const SelectModeView: React.FC<{ onModeChange: (mode: 'send' | 'receive') => void }> = ({
  onModeChange,
}) => (
  <div className="space-y-4">
    ...
    <div className="pt-4 border-t border-gray-200">
      <p className="text-xs text-gray-500 text-center">
        Note: Offline P2P sharing is under active development. For now, please use Export/Import
        feature in Settings.
      </p>
    </div>
  </div>
...
const SendModeView: React.FC<{
  ...
  onStopHosting: () => void;
  onCopyCode: () => void;
}> = ({ shareCode, isHosting, copied, onBack, onStartHosting, onStopHosting, onCopyCode }) => (
  <div className="space-y-4">
      ...
      {!isHosting ? (
        <>
          <p className="text-gray-600">
            Start sharing to generate a connection code that other devices can use to receive your
            data.
          </p>
          <button
            onClick={onStartHosting}
            className="w-full px-6 py-3 bg-linear-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white rounded-xl transition-all font-semibold shadow-lg"
          >
            Start Sharing
          </button>
        </>
...
const ReceiveModeView: React.FC<{
  connectionCode: string;
  isConnecting: boolean;
  onBack: () => void;
  onCodeChange: (code: string) => void;
  onJoin: () => void;
}> = ({ connectionCode, isConnecting, onBack, onCodeChange, onJoin }) => (
  <div className="space-y-4">
    <button onClick={onBack} className="text-sm text-gray-600 hover:text-gray-900 mb-4">
      ← Back
    </button>
    <div className="space-y-4">
      ...
      <button
        ...
        className="w-full px-6 py-3 bg-linear-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700 text-white rounded-xl transition-all font-semibold shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"
      >
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl p-8 max-w-md w-full shadow-2xl border border-gray-200 max-h-[90vh] overflow-y-auto">
        ...
        {mode === 'select' && <SelectModeView onModeChange={setMode} />}
        {mode === 'send' && (
          <SendModeView
            shareCode={shareCode}
            isHosting={isHosting}
        ...
        {mode === 'receive' && (
          <ReceiveModeView
            ...
            onBack={() => setMode('select')}
            onCodeChange={setConnectionCode}
